...
class ListUsers extends ListRecords
{
    ...
    public function getTabs(): array
    {
        ...
        $tabs = [
            ...
            'admin' => Tab::make()->label('Administrador')->query(fn ($query) => $query->with('roles')->whereRelation('roles', 'name', '=', 'admin')),
        ];

        if ($user->isSuperAdmin()) {
            $tabs['superadmin'] = Tab::make()->label('Super-Administrador')->query(fn ($query) => $query->with('roles')->whereRelation('roles', 'name', '=', config('filament-shield.super_admin.name')));
        }
